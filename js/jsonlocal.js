...
//var mapcoor = new google.maps.LatLng(+data[i].COORDENADAX+, +data[i].COORDENADAY+);
     ...
     $("#lista").listview("refresh");
      ...
      $("#lista").on("click", "div a", function(e){
        ...
        $("#Detalles").append("<br /><strong>"+ obj.NOMCORTO +"</strong><i id=\"despuesclick\" class=\"\"></i><i id=\"fav\" class=\"zmdi zmdi-grid\"></i><label id=\"recordlocal\"></label><label id=\"recordlocal2\"></label><br>");

        $("#Detalles").append("<div class=\"perfil\"><br>PUNTAJE:<b>" + obj.PUNTAJE +"</b> IDGRUPO:<b>" + obj.IDGRUPO +"</b></div>");

        $("#Detalles").append("<input type=\"hidden\" id=\"id\" value="+ obj.IDEMPRESA +"><input type=\"range\" name=\"slider-3\" id=\"puntaje\" data-track-theme=\"a\" data-theme=\"b\" value=\"5\" min=\"0\" max=\"5\" data-highlight=\"true\"><div id=\"btn-list\"><button id=\"guardardatos\">Califica el establecimiento</button></div>");
...
$(document).on('pageinit', function() {
  ...
  localStorage.setItem("local", JSON.stringify(data[1]));
...
$(document).ready(function(){
  var recordlocal = localStorage.getItem("local");

    document.getElementById("recordlocal").innerHTML = recordlocal;
    document.getElementById("recordlocal2").innerHTML = recordlocal2;
});
